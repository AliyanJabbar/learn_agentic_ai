...
def main():
    ...
    precise_agent = base_agent.clone(
        ...
        instructions="You are a precise, focused assistant.",
    ...
    print(
        "Shared clone tools:", len(shared_clone.tools)
    )  # 2 (shared!) #no overriding of tools

    # Create independent clone
    independent_clone = original_agent.clone(
        name="IndependentClone",
        tools=[calculate_area],  # New list # overriding of tools at any cost
    ...
    print("Shared clone tools:", len(shared_clone.tools))  # 3 (shared!)
